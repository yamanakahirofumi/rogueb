...
本プロジェクトは、マイクロサービスアーキテクチャを採用しています。

このアーキテクチャを採用する目的は以下の通りです：
- **スケールアウトの容易性**: 負荷の高い特定のサービスのみを個別に拡張できるようにするため。
- **影響範囲の局所化とバリアント開発の促進**: サービス間の結合を疎にすることで、変更の影響範囲を小さくし、新しいバリアントや機能が開発されやすくするため。

## 1. モジュール構造

プロジェクトは、マルチモジュール構成の Spring Boot アプリケーション群で構成されています。リアクティブプログラミング（Spring WebFlux）を採用し、スケーラビリティと非同期処理を重視した設計となっています。
以下の役割を持つ複数のモジュールに分割されています。
