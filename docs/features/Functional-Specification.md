...
### 5.1 管理者の利益（略奪と収集）
- **入場料による収益**: 管理者（ダンジョンオーナー）は、自身のダンジョンへの**入場料 (Entry Fee)** を設定できます。プレイヤーが入場する際、この金額がプレイヤーの所持金から差し引かれ、管理者の所持金に加算されます。
...
### 5.2 ダンジョンの運営
- **クリア報酬**:
    - 管理者はプレイヤーを誘い込むために、自身の所持品（ゴールドまたはアイテム）からクリア報酬を設定することができます。
    - プレイヤーが事前に設定された**クリア条件 (`clearCondition`)**（最深部への到達、特定のボスの撃破など）を満たした際に、設定された報酬がプレイヤーへ付与されます。
    - 報酬として設定されたアイテムは、設定時に管理者のインベントリから一時的に「報酬用プール」へ移動し、クリアされるまで管理者は使用できません。
