...
- **プロパティ:**
    - `itemForfeitureType`: アイテム没収の種類（「全アイテム没収」「ランダムで数個没収」「没収なし」）。
    - `goldLossType`: ゴールド喪失の種類（「全額喪失」「固定額の喪失」「所持金の一定パーセントを喪失」）。
    - `goldLossValue`: ゴールド喪失の具体的な値（固定額またはパーセンテージ）。
    - `statusResetType`: ステータスリセットの種類（「レベルが 1 に戻る」「経験値が一定量減少する」「減少なし」）。
