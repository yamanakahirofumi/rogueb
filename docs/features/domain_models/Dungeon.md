...
- **主要なプロパティ:**
    ...
    - `interventionPoints`: 管理者がリアルタイム介入に使用する現在のポイント (IP)。詳細は [ダンジョン構築・運営システム](../Dungeon-Construction-System.md) を参照。
...
- **値:**
    - `DARKNESS`: プレイヤーの視界が通常より狭くなる。
    - `MIASMA`: 自然回復が無効化され、数歩ごとに微量のダメージを受ける。
    - `MAGIC_INTERFERENCE`: 魔法や杖の消費 MP が 2 倍になる。
    - `HEAVY_GRAVITY`: 移動時のスタミナ消費が 2 倍になる。
