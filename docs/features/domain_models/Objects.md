...
### 補足: 効果・補正の記述について
アイテムテーブルの「効果・補正」列では、可読性のために簡略化した記法（例: `atk: +3`）を使用しています。これらは実装上、**[標準メタデータ仕様](../Standard-Metadata-Specification.md)** で定義された構造化されたメタデータとして `ThingInstance.metadata` または `meta_effects` 配列に格納されます。

- **ステータス補正**: `atk`, `def`, `magicAtk` 等のキーとして直接格納されます。
- **特殊効果**: `meta_effects` 配列内にエフェクト ID とパラメータの形式で格納されます（例: `HEAL_HP: 30` → `{"id": "HEAL_HP", "params": {"amount": 30}}`）。
...
| `swap_stick` | 場所替えの杖 | `STICK` | None | 1000 | 2 | 500 | `/` | `SWAP_POSITION` |
| `fire_stick` | 火炎の杖 | `STICK` | Fire | 1200 | 2 | 500 | `/` | `DEAL_DAMAGE: 20` |
| `sleep_stick` | 睡眠の杖 | `STICK` | None | 1000 | 2 | 500 | `/` | `ADD_STATUS: SLEEP` |
