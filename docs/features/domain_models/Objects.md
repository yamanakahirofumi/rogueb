...
- **フィールド:**
    ...
    - `tier`: オブジェクトのティア。
    - `metadata`: インスタンス固有の動的データ（Map<String, Object>）。詳細は後述の「予約済みメタデータキー」を参照してください。
...
| `egg` | 卵 | `OTHER` | 0 | 1 | `*` | 孵化するとモンスターが生まれる卵。 |

## 5. 予約済みメタデータキー
`ThingInstance.metadata` にて使用される予約済みのキーを定義します。

### 5.1 [世界間連携システム](../World-Interoperability-System.md)
- `meta_visual_asset_id`: アセット参照 ID。
- `meta_sprite_data`: Base64 エンコードされたスプライト画像データ。
- `meta_effects`: 標準化されたエフェクト ID のリスト。

### 5.2 [戦闘システム](../Combat-System.md)
- `atk`, `def`, `magicAtk`, `magicDef`, `dex`, `mnd`, `maxHp`, `maxMp`, `maxStamina`: ステータス補正値。

### 5.3 [モンスター繁殖システム](../Monster-Breeding-System.md)
- `typeId`: 孵化対象のモンスター種族 ID。
- `parentAId`, `parentBId`: 両親のインスタンス ID。
- `incubationSteps`: 孵化に必要な総歩数。
- `currentSteps`: 現在の累積歩数。
