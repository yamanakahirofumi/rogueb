...
| ID | 名称 | カテゴリ | 標準価格 | ティア | 表示 | 効果・補正 |
| :--- | :--- | :--- | :---: | :---: | :---: | :--- |
| `wooden_sword` | 木の剣 | `WEAPON` | 100 | 1 | `)` | `atk: 3` |
| `leather_armor` | 皮の鎧 | `ARMOR` | 150 | 1 | `[` | `def: 2` |
| `healing_potion` | 癒しの薬 | `POTION` | 50 | 1 | `!` | HP を 30 回復する。 |
| `bread` | パン | `FOOD` | 30 | 1 | `%` | スタミナを 50 回復する。 |
| `capture_capsule` | 捕獲カプセル | `OTHER` | 500 | 1 | `0` | モンスターを捕獲する。 |
| `great_capsule` | 高性能カプセル | `OTHER` | 1500 | 2 | `0` | 捕獲成功率が少し高いカプセル。 |
| `ultra_capsule` | 最高性能カプセル | `OTHER` | 5000 | 3 | `0` | 捕獲成功率が非常に高いカプセル。 |
| `elemental_capsule` | 属性カプセル | `OTHER` | 3000 | 2 | `0` | 対象の属性と一致する場合、非常に高い効果を発揮する。 |
| `monster_soul_gem` | モンスターソウルジェム | `OTHER` | 1000 | 2 | `*` | モンスターに変身し、乱入する。 |
| `id_scroll` | 識別の巻物 | `SCROLL` | 200 | 1 | `?` | アイテムを識別する。 |
| `uncurse_scroll` | 解呪の巻物 | `SCROLL` | 300 | 1 | `?` | 装備の呪いを解く。 |
| `loyalty_cookie` | 懐きクッキー | `FOOD` | 400 | 1 | `%` | モンスターの忠誠度を 20 上げる。 |
| `wonder_candy` | 不思議な飴 | `FOOD` | 2000 | 3 | `%` | モンスターにレベル 1 分の経験値を与える。 |
| `fire_stone` | 炎の石 | `MATERIAL` | 500 | 2 | `*` | 特定のモンスターを進化させるための触媒。 |
| `bounty_hunter_proof` | 賞金稼ぎの証 | `OTHER` | 2000 | 3 | `"` | 強力な賞金稼ぎを倒した証。高額で取引される。 |
| `egg` | モンスターの卵 | `OTHER` | 1000 | 2 | `*` | 孵化させることでモンスターが生まれる卵。 |
| `dragon_killer` | ドラゴンキラー | `WEAPON` | 4000 | 3 | `)` | `atk: 12`, `slayer: DRAGON` |
| `beast_buster` | ビーストバスター | `WEAPON` | 2500 | 2 | `)` | `atk: 8`, `slayer: BEAST` |
| `demon_slayer` | 魔物斬り | `WEAPON` | 3500 | 3 | `)` | `atk: 10`, `slayer: DEMON` |
| `silver_sword` | 銀の剣 | `WEAPON` | 1500 | 2 | `)` | `atk: 6`, `slayer: UNDEAD` |
| `sample_ring` | 試作の指輪 | `RING` | 500 | 1 | `=` | 識別の動作確認用サンプル。 |
| `sample_stick` | 試作の杖 | `STICK` | 800 | 1 | `/` | 識別の動作確認用サンプル。 |
| `stone_floor` | 石の床 | `MATERIAL` | 50 | 1 | `.` | 建築用資材。 |
| `waterway` | 水路 | `MATERIAL` | 100 | 1 | `~` | 建築用資材。進入不可（一部除く）。 |
| `lava` | 溶岩 | `MATERIAL` | 200 | 2 | `~` | 建築用資材。進入時にダメージ。 |
| `strong_wall` | 頑丈な壁 | `MATERIAL` | 150 | 1 | `#` | 建築用資材。掘削困難な壁。 |
| `door` | 扉 | `MATERIAL` | 100 | 1 | `+` | 建築用資材。開閉可能な境界。 |
| `breakable_wall` | 壊れる壁 | `MATERIAL` | 80 | 1 | `#` | 建築用資材。攻撃で破壊可能。 |
| `shop_counter` | ショップカウンター | `MATERIAL` | 500 | 2 | `S` | ショップ設置用の資材。 |
| `healing_spring` | 回復の泉 | `MATERIAL` | 1000 | 3 | `W` | 建築用資材。HP/MPを回復する。 |
| `trap_pitfall` | 落とし穴の種 | `OTHER` | 200 | 1 | `^` | 設置することで「落とし穴」になる。 |
| `trap_poison_arrow` | 毒矢の罠の種 | `OTHER` | 150 | 1 | `^` | 設置することで「毒矢の罠」になる。 |
| `trap_landmine` | 地雷の種 | `OTHER` | 300 | 2 | `^` | 設置することで「地雷」になる。 |
| `trap_sleep_gas` | 睡眠ガスの種 | `OTHER` | 250 | 2 | `^` | 設置することで「睡眠ガス」になる。 |
| `trap_unequip` | 装備外しの罠の種 | `OTHER` | 400 | 3 | `^` | 設置することで「装備外しの罠」になる。 |
| `trap_hunger` | 空腹の罠の種 | `OTHER` | 100 | 1 | `^` | 設置することで「空腹の罠」になる。 |
| `trap_summon` | 召喚の罠の種 | `OTHER` | 500 | 3 | `^` | 設置することで「召喚の罠」になる。 |
