...
| `egg` | 卵 | `OTHER` | 1000 | 2 | `e` | モンスターの卵。孵化させることで仲間を増やせる。 |

## 5. 予約済みメタデータキー (Reserved Metadata Keys)
`ThingInstance.metadata` において、特定のシステムで使用される予約済みのキーを定義します。

### 5.1 繁殖・孵化システム (`egg` 関連)
詳細は [モンスター繁殖システム](../Monster-Breeding-System.md) を参照してください。

- `typeId` (String): 孵化するモンスターの種族 ID。
- `parentAId` (String): 親 A のインスタンス ID。
- `parentBId` (String): 親 B のインスタンス ID。
- `incubationSteps` (Integer): 孵化に必要な総歩数。
- `currentSteps` (Integer): 現在の累積歩数。

### 5.2 戦闘システム (`WEAPON`, `ARMOR` 等)
詳細は [戦闘システム](../Combat-System.md) を参照してください。

- `atk`, `def`, `magicAtk`, `magicDef`, `dex`, `mnd`, `maxHp`, `maxMp`, `maxStamina`: ステータス補正値。
- `slayer` (String): 特定種族への特効カテゴリ（例: `DRAGON`）。

### 5.3 世界間連携システム
詳細は [世界間連携システム](../World-Interoperability-System.md) を参照してください。

- `meta_visual_asset_id` (String): アセット参照 ID。
- `meta_sprite_data` (String): Base64 エンコードされたスプライト画像データ。
- `meta_effects` (List): 標準化されたエフェクトリスト。
