...
- **フィールド:**
    ...
    - `tier`: オブジェクトのティア。
    - `metadata`: インスタンス固有の動的データ（Map<String, Object>）。卵の孵化歩数や親情報などに使用されます。また、[世界間連携システム](../World-Interoperability-System.md) における「未知のアイテム」を扱うための予約キー（`meta_visual_asset_id`, `meta_sprite_data`, `meta_effects`）が定義されています。
