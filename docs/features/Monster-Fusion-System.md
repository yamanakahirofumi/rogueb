...
### 4.4 融合におけるスキル継承と制限
- **スキル枠の制限**: 合体後のベースモンスターが保持できるスキル（`skillIds`）は、**最大 4 つ**に制限されます。
- **スキル超過時の挙動**: 系統継承合体または特殊進化合体のプロセス中に、継承したスキルとベースモンスターが元から保持していたスキルの合計が 5 つ以上になる場合、プレイヤーは手動で「残す 4 つのスキル」を選択しなければなりません（選択しなかったスキルは消滅します）。

### 4.5 融合における厳格なバランス制限
ゲームバランスを維持するため、融合システムには以下の厳格な制限が適用されます。
- **累計融合回数の制限 (fusionCount)**:
  - 1体のモンスターがベース（Base）として融合を行える回数は、**最大 5 回**に制限されます。
  - この回数は個体の `fusionCount` プロパティによって記録・管理されます。累計融合回数が 5 回に達したモンスターは、それ以上ベースとして融合を実行することはできません（素材としての使用は可能です）。
- **継承ステータスの上限 (Inherited Stats Cap)**:
  - 融合によってベースモンスターに加算される継承ステータス補正（`inheritedStatus`）は、その種族の **Lv 1 基本ステータス（種族基本値）の 50%** を上限（キャップ）とします。これを超える値が計算された場合でも、上限値までに切り捨てられます。
- **個体特性数の上限 (Individual Traits Cap)**:
  - 融合によって素材モンスターから継承される個体固有の特性（パッシブ特性）は、1体の個体につき**最大 2 つ**に制限されます。すでに 2 つの個体特性を持つベースモンスターがさらに融合を行う場合、既存の特性を上書きして選択するか、新たな特性の継承を諦める必要があります。
  - **重要 (種族特性との区別)**: 種族固有の特性（`MonsterDomain.traits`）はこの制限のカウント（最大 2 つ）に含まれません。本上限はあくまで個体ごとの `MonsterInstanceDomain.traits` に保存されている「個体特性（継承・突然変異特性）」のみに適用されます。詳細は [モンスター特性システム](../Monster-Trait-System.md) を参照してください。
...
- **追加された標準プロパティ**:
  - `fusionCount` (Integer): ベース個体（Base）としての累計融合回数を記録するフィールド。初期値は `0` であり、融合を実行するたびに `1` 加算されます（最大値は `5`）。
