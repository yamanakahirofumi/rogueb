...
### 2.4 累積融合回数の制限 (Fusion Count Limit)
- **ベース個体の制限**: 1体のモンスター個体がベースとして融合を行える累積回数は、系統継承合体・特殊進化合体を問わず**最大 5 回**に制限されます。
- **素材個体の制限**: 素材として消費（消失）されるモンスターには累積融合回数の制限はありません。
- **永続化**: 累積融合回数は、`MonsterInstanceDomain` の `fusionCount` フィールドに記録されます。
...
- **ステータス継承の上限 (Inherited Status Cap)**:
  - 融合によってベースモンスターに加算される累積の `inheritedStatus[stat]` は、過度なステータスインフレを防ぐため、そのモンスター種族（`MonsterDomain`）の **レベル 1 時の基本ステータス（`baseStatus`）の 50%** を上限（キャップ）とします。
...
- **個体特性枠の上限 (Individual Traits Cap)**:
  - 融合によってベースモンスターが保持できる固有・パッシブ特性（種族標準の特性を除く、個体固有の特性）の数は、**最大 2 個**に制限されます。すでに上限に達している場合は、新たに継承する特性を破棄するか、既存の個体特性を上書きして忘れる必要があります。
...
### 4.4 スキル継承とスキル枠制限 (Skill Inheritance and Limits)
- **スキル枠の制限 (Max 4 Skills)**:
  - モンスター個体が保持できるスキルの最大数は **4 つ**です。
  - 融合の際、ベース個体がすでに習得しているスキルと、素材個体から継承・移植しようとするスキルの合計が 4 つを超える場合、プレイヤーは手動で「現在のスキルを忘れて新しいスキルを継承する」か「新しいスキルの継承を諦める」かを**手動で選択する**必要があります（繁殖、孵化、進化、レベルアップ時の挙動と同様です）。
...
- **スキーマ定義と制限**:
  - **`fusionCount` (Integer)**:
    ベース個体としての累計融合回数を記録する必須フィールドです。初期値は `0` であり、ベースとして融合を完了するたびに `1` 加算され、**最大 5** に制限されます。
