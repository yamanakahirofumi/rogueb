...
- **介入ポイント (IP)**:
    ...
    - 各アクションごとに消費 IP が定義されます。
- **介入アクション**:
    - **増援の召喚**: ストックしているモンスターを、プレイヤーの視界外に即座に配置します（消費 IP: 大）。
    - **トラップの手動発動**: プレイヤーが踏んでいないトラップを、管理者の意思で強制的に作動させます（消費 IP: 中）。
    - **メッセージ送信**: 攻略中のプレイヤーに対し、特定のメッセージ（天の声）を表示します（消費 IP: 低）。
