...
- 技術スタック: Spring Boot 4.0.3 (Target) / Spring WebFlux (Reactor Mono/Flux), Java 21, Maven マルチモジュール
...
%% Monster
    Monster --> ObjectsClient

%% EconomicSystem
    EconomicSystem --> BookOfAdventureClient
...
- GET `/api/user/id/{userId}/items`
  ...
  - 戻り値: `Flux<String>`
- GET /api/user/id/{userId}/knowledge
  - 目的: ユーザーのアイテム知識（識別状況）一覧を取得
  - 戻り値: `Flux<PlayerKnowledgeDomain>`
- POST /api/user/id/{userId}/knowledge
  - 目的: ユーザーに新しいアイテム知識を記録
  - リクエストボディ: `PlayerKnowledgeDomain`
  - 戻り値: `Mono<String>` (userId)
...
- POST `/api/objects/instance/{id}/`
  ...
  - リクエストボディ: `String description`
  - 戻り値: `Mono<ThingInstance>`
- GET /api/objects/appearance/{worldId}/{typeId}
  - 目的: ワールドごとのアイテムタイプの外見名（未識別名）を取得
  - 戻り値: `Mono<String>`
- POST /api/objects/instance/{id}/identify
  - 目的: 特定のアイテムインスタンスを識別済みに更新
  - 戻り値: `Mono<ThingInstance>`
...
- GET `/api/monster/type/{typeId}`
  - 目的: モンスターの種族情報の取得
  - 戻り値: `Mono<MonsterDomain>`
- GET `/api/monster/instance/{instanceId}`
  - 目的: 特定のモンスター個体情報の取得
  - 戻り値: `Mono<MonsterInstanceDomain>`
- POST `/api/monster/breeding`
  - 目的: モンスターの繁殖（卵の生成）
  - リクエストボディ: `Map<String, String>` (parentAId, parentBId)
  - 戻り値: `Mono<ThingInstance>` (生成された卵)
- POST `/api/monster/hatch/{instanceId}`
  - 目的: 卵の孵化
  - 戻り値: `Mono<MonsterInstanceDomain>` (孵化したモンスター)

### EconomicSystem API

- GET `/api/economic/circulation/{typeId}`
  - 目的: アイテムの流通量と基本価格の取得
  - 戻り値: `Mono<ItemCirculationDomain>`
- GET `/api/economic/shop/{shopId}`
  - 目的: ショップ情報の取得
  - 戻り値: `Mono<ShopDomain>`
- POST `/api/economic/trade`
  - 目的: ショップでの売買処理
  - リクエストボディ: `TransactionHistoryDomain`
  - 戻り値: `Mono<TransactionHistoryDomain>`
