...
#### 3.1.1 モンスター忠誠度の扱い (Monster Loyalty Policy)
モンスターのマイグレーションに伴い、忠誠度 (`loyalty`) および最終更新日時 (`lastLoyaltyUpdate`) も転送されます。

- **忠誠度の維持**: 信頼レベルが「共有」または「引き継ぐ」の場合、現在の忠誠度をそのまま維持します。
- **放置減衰の精算**: エクスポート時に、その時点までの「放置による減少（Lazy Calculation）」を確定させてからパッケージ化します。これにより、移動先サーバーでの `lastLoyaltyUpdate` は移動完了時のタイムスタンプにリセットされます。
- **ボーナスの制限**: 信頼度が低いサーバーからの移動個体については、移動先サーバーのポリシーにより、忠誠度によるステータス補正や「食いしばり」が一時的に無効化、あるいは制限（例：最大でも 1.03 倍まで）される場合があります。
...
- **特殊効果 (`meta_effects`)**:
    - 標準化されたエフェクト ID。詳細は **[標準メタデータ仕様](./Standard-Metadata-Specification.md)** を参照してください。

### 4.2 標準化エフェクトリスト (Standardized Effect List)
未知のアイテムの効果を移動先サーバーで再現するための、共通エフェクト ID の定義です。詳細なパラメータ定義は **[標準メタデータ仕様](./Standard-Metadata-Specification.md)** を参照してください。

| エフェクト ID | 概要 | 備考 |
| :--- | :--- | :--- |
| `HEAL_HP` | HP を回復する | ポーション、食料等 |
| `HEAL_MP` | MP を回復する | ポーション等 |
| `HEAL_STAMINA` | スタミナを回復する | 食料、薬等 |
| `ADD_STATUS` | 状態異常を付与する | 武器、巻物、杖等 |
| `REMOVE_STATUS` | 状態異常を解除する | 薬草、巻物等 |
| `STAT_BOOST` | ステータスを永続/一時強化 | 装備品（パッシブ）、薬等。 |
| `TELEPORT` | 別の座標へワープする | 巻物、杖、トラップ等 |
| `EXPLOSION` | 周囲に爆発ダメージ | 巻物、杖等 |
| `REVEAL_MAP` | マップの視界を広げる | 巻物等 |
| `IDENTIFY` | アイテムを識別する | 巻物等 |
| `REMOVE_CURSE` | 呪いを解除する | 巻物等 |
| `DEAL_DAMAGE` | 対象に直接ダメージ | 杖、巻物等 |
| `HOLY_DAMAGE` | 聖なるダメージを与える | 聖なる武器等 |
...
### 4.4 未知のモンスターの持ち込み (Unknown Monster Protocol)
移動先サーバーに定義されていないモンスター種族（カスタムモンスター等）を連れて移動した際の挙動を定義します。

#### 4.4.1 データ継承 (Data Inheritance)
移動先サーバーに該当する `monsterId` が存在しない場合、以下のメタデータ一式をキャラクターデータと共に転送し、移動先サーバーで一時的な `MonsterDomain` および `MonsterInstanceDomain` として扱います。

- **種族情報**:
    - `name` (名称), `display` (表示文字), `type` (カテゴリ: `SLIME`, `BEAST` 等), `attribute` (属性)
    - `baseStats`: `hp`, `mp`, `atk`, `def`, `magicAtk`, `magicDef`, `dex`, `mnd` の各基本値。
    - `traits`: 保持している特性 ID のリスト。
    - `baseActionInterval`: 基本行動間隔。
- **個体情報**:
    - `level` (現在のレベル), `experience` (累積経験値), `loyalty` (忠誠度)。
    - `skillIds`: 習得しているスキル ID のリスト。
        - 移動先サーバーに存在しないスキルの扱いは、「4.2 標準化エフェクトリスト」に基づき、可能な限り互換性のある効果に置き換えられます。

#### 4.4.2 動作保証 (Behavioral Guarantees)
- **AI 挙動**: 移動先サーバーの標準的なモンスター AI ロジックで動作します。特性（`TRACKING` 等）がある場合は、それに基づいた特殊な挙動も反映されます。
- **成長と進化**: 未知のモンスターであっても、移動先サーバー内で経験値を獲得しレベルアップすることが可能です。ただし、進化先種族が移動先サーバーに存在しない場合、進化は制限される場合があります。
