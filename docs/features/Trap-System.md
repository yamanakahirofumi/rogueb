...
| ID | 名称 | ティア | 配置コスト | 作動時の効果 | 露出状態への遷移 |
| :--- | :--- | :---: | :---: | :--- | :--- |
| 1 | **落とし穴** | 1 | 50 | 下の階層へ強制的に移動し、**現在 HP の 10%** のダメージを受ける。 | 作動後に消滅 |
| 2 | **毒矢の罠** | 1 | 30 | 正面から矢が飛び出し、**固定 5 ダメージ**と「毒」状態を与える。 | 作動後に露出 |
| 3 | **地雷** | 2 | 100 | 周囲 8 マスのエンティティに**現在 HP の 50%** のダメージを与える。 | 作動後に消滅 |
| 4 | **睡眠ガス** | 2 | 80 | 周囲にガスを噴射し、「睡眠」状態を与える。 | 作動後に露出 |
| 5 | **装備外しの罠** | 3 | 150 | 装備している武器・防具を強制的に解除する。 | 作動後に露出 |
| 6 | **空腹の罠** | 1 | 20 | スタミナ（満腹度）を **20 減少**させる。 | 作動後に露出 |
| 7 | **召喚の罠** | 3 | 200 | 周囲に複数のモンスターをランダムに召喚する。 | 作動後に露出 |
| 8 | **火炎の罠** | 2 | 70 | **火属性のダメージ**を与える。 | 作動後に露出 |
| 9 | **氷結の罠** | 2 | 75 | **氷属性のダメージ**と「鈍足」状態を与える。 | 作動後に露出 |
| 10 | **電撃の罠** | 3 | 120 | **雷属性のダメージ**と「麻痺」状態を与える。 | 作動後に露出 |
