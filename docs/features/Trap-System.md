...
| 名称 | 作動時の効果 | 露出状態への遷移 |
| :--- | :--- | :--- |
| **落とし穴** | 下の階層へ強制的に移動し、**現在 HP の 10%** のダメージを受ける。 | 作動後に消滅 |
| **毒矢の罠** | 正面から矢が飛び出し、**固定 5 ダメージ**と「毒」状態を与える。 | 作動後に露出 |
| **地雷** | 周囲 8 マスのエンティティに**現在 HP の 50%** のダメージを与える。 | 作動後に消滅 |
| **睡眠ガス** | 周囲にガスを噴射し、「睡眠」状態を与える。 | 作動後に露出 |
| **装備外しの罠** | 装備している武器・防具を強制的に解除する。 | 作動後に露出 |
| **空腹の罠** | スタミナ（満腹度）を **20 減少**させる。 | 作動後に露出 |
