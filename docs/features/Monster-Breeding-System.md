...
- **継承枠**: 最大 **2 つ**。
- **選出ルール**:
    1. 両親が習得しているすべてのスキル（`skillIds`）をリストアップします。
    2. リストからランダムに 2 つが選ばれます。同一のスキルが選ばれた場合は、1 つのみ継承されます。
- **継承確率**: リスト内の各スキルについて、個別に **30%** の確率で継承判定が行われます。
- **突然変異スキル**: 継承枠に空きがある場合、**5%** の確率で両親が持っていないスキル（そのカテゴリに属するランダムなスキル）を 1 つ習得した状態で生まれます。
