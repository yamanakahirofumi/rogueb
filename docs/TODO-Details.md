...
### [x] モンスター融合システムにおける制限仕様の追加とドメイン定義の拡張
- **概要**: 融合システムの実装精度向上とバランス調整のため、融合回数制限や継承ステータスの上限、および特性枠の制限を明記し、ドメインモデルへ反映。
- **解決策**: `Monster-Fusion-System.md`, `Monster.md`, `Monster-MongoDB.md` にて、累計融合回数の制限（最大5回）、継承ステータス上限（種族Lv1基本値の50%）、個体特性枠上限（最大2個）を追記し、`fusionCount` フィールドをドメインおよび MongoDB スキーマに追加。さらに、融合がスキル上限（4つ）超過時の手動選択トリガーとなることを明記。
